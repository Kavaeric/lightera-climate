...
function generateLatitudeSubdivisions(
	...
	console.log(`generateLatitudeSubdivisions: [${latitudes.map(lat => lat.toFixed(1)).join(', ')}]`)
 ...
 */
function generateLongitudeSubdivisions(longitudeCount: number, startLongitude: number, endLongitude: number, returnStart: boolean = false, returnEnd: boolean = false): number[] {
	const longitudes = []
	...
	console.log(`generateLongitudeSubdivisions: [${longitudes.map(lon => lon.toFixed(1)).join(', ')}]`)
