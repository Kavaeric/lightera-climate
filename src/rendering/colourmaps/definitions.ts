/**
 * Colourmap definitions for data visualisation
 * Each colourmap defines colour ramps and underflow/overflow colours
 */

import * as THREE from 'three';
import type { ColourmapDefinition } from './ColourmapTexture';

/**
 * Greyscale colourmap - simple black to white
 */
export const COLOURMAP_GREYSCALE: ColourmapDefinition = {
  name: 'greyscale',
  colours: [
    new THREE.Vector3(0.0, 0.0, 0.0), // Black
    new THREE.Vector3(1.0, 1.0, 1.0), // White
  ],
  underflowColour: new THREE.Vector3(0.0, 0.0, 0.2), // Navy blue for underflow
  overflowColour: new THREE.Vector3(1.0, 0.0, 1.0), // Magenta for overflow
};

/** "Yel15" via https://sciviscolor.org/ */
export const COLOURMAP_YELLOW_YEL15: ColourmapDefinition = {
  name: 'yellow_yel15',
  colours: [
    new THREE.Vector3(0.301961, 0.047059, 0.090196),
    new THREE.Vector3(0.4, 0.039216, 0.058824),
    new THREE.Vector3(0.501961, 0.054902, 0.031373),
    new THREE.Vector3(0.6, 0.121569, 0.023529),
    new THREE.Vector3(0.678431, 0.180392, 0.011765),
    new THREE.Vector3(0.760784, 0.239216, 0.0),
    new THREE.Vector3(0.8, 0.305882, 0.0),
    new THREE.Vector3(0.890196, 0.411765, 0.019608),
    new THREE.Vector3(0.909804, 0.486275, 0.035294),
    new THREE.Vector3(0.921569, 0.545098, 0.054902),
    new THREE.Vector3(0.929412, 0.607843, 0.094118),
    new THREE.Vector3(0.941176, 0.678431, 0.14902),
    new THREE.Vector3(0.94902, 0.741176, 0.219608),
    new THREE.Vector3(0.960784, 0.803922, 0.286275),
    new THREE.Vector3(0.968627, 0.862745, 0.388235),
    new THREE.Vector3(0.968627, 0.905882, 0.486275),
    new THREE.Vector3(0.980392, 0.945098, 0.576471),
    new THREE.Vector3(0.980392, 0.968627, 0.654902),
    new THREE.Vector3(0.988235, 0.988235, 0.741176),
    new THREE.Vector3(0.984314, 0.988235, 0.843137),
    new THREE.Vector3(1.0, 1.0, 0.988235),
    new THREE.Vector3(1.0, 1.0, 0.988235),
  ],
  underflowColour: new THREE.Vector3(0.0, 0.0, 0.2), // Navy blue
  overflowColour: new THREE.Vector3(1.0, 0.0, 1.0), // Magenta
};

/**
 * "BlueB1" via https://sciviscolor.org/
 */
export const COLOURMAP_BLUE_B1: ColourmapDefinition = {
  name: 'blue_b1',
  colours: [
    new THREE.Vector3(0.882353, 0.980392, 0.964706), // x=0.0
    new THREE.Vector3(0.690196, 0.921569, 0.913725), // x=0.1
    new THREE.Vector3(0.552941, 0.823529, 0.85098), // x=0.2
    new THREE.Vector3(0.478431, 0.737255, 0.8), // x=0.3
    new THREE.Vector3(0.384314, 0.596078, 0.701961), // x=0.4
    new THREE.Vector3(0.321569, 0.490196, 0.639216), // x=0.5
    new THREE.Vector3(0.262745, 0.392157, 0.580392), // x=0.6
    new THREE.Vector3(0.207843, 0.301961, 0.521569), // x=0.7
    new THREE.Vector3(0.156863, 0.207843, 0.45098), // x=0.8
    new THREE.Vector3(0.121569, 0.145098, 0.4), // x=0.9
    new THREE.Vector3(0.098039, 0.086275, 0.34902), // x=1.0
  ],
  underflowColour: new THREE.Vector3(0.0, 0.0, 0.0),
  overflowColour: new THREE.Vector3(0.0, 1.0, 1.0),
};

/**
 * "c16" via https://sciviscolor.org/
 */
export const COLOURMAP_TEAL_C16: ColourmapDefinition = {
  name: 'teal_c16',
  colours: [
    new THREE.Vector3(0.94902, 1.0, 0.980392),
    new THREE.Vector3(0.901961, 1.0, 0.94902),
    new THREE.Vector3(0.831373, 0.980392, 0.917647),
    new THREE.Vector3(0.768627, 0.960784, 0.894118),
    new THREE.Vector3(0.698039, 0.929412, 0.862745),
    new THREE.Vector3(0.639216, 0.901961, 0.843137),
    new THREE.Vector3(0.588235, 0.870588, 0.827451),
    new THREE.Vector3(0.541176, 0.831373, 0.803922),
    new THREE.Vector3(0.501961, 0.8, 0.796078),
    new THREE.Vector3(0.454902, 0.745098, 0.760784),
    new THREE.Vector3(0.419608, 0.694118, 0.729412),
    new THREE.Vector3(0.380392, 0.631373, 0.690196),
    new THREE.Vector3(0.345098, 0.580392, 0.658824),
    new THREE.Vector3(0.317647, 0.52549, 0.619608),
    new THREE.Vector3(0.305882, 0.470588, 0.580392),
    new THREE.Vector3(0.290196, 0.415686, 0.529412),
    new THREE.Vector3(0.278431, 0.352941, 0.470588),
    new THREE.Vector3(0.258824, 0.298039, 0.4),
    new THREE.Vector3(0.235294, 0.247059, 0.329412),
    new THREE.Vector3(0.180392, 0.176471, 0.231373),
  ],
  underflowColour: new THREE.Vector3(0.0, 0.0, 0.0),
  overflowColour: new THREE.Vector3(1.0, 0.0, 1.0),
};

/**
 * Specific colourmap for water state
 * 0.0 = solid, dark greyish blue
 * 0.5 = liquid, bright blue
 * 1.0 = vapour, light greyish blue
 * Underflow is black, overflow is magenta
 */
export const COLOURMAP_WATER_STATE: ColourmapDefinition = {
  name: 'water_state',
  colours: [
    new THREE.Vector3(0.2, 0.2, 0.4), // Solid, navy blue
    new THREE.Vector3(0.0, 0.4, 1.0), // Liquid, aqua
    new THREE.Vector3(0.7, 0.7, 0.9), // Vapour, cyan
  ],
  underflowColour: new THREE.Vector3(0.0, 0.0, 0.0),
  overflowColour: new THREE.Vector3(1.0, 0.0, 1.0),
};

/**
 * "sd" via https://sciviscolor.org/
 */
export const COLOURMAP_BLUE_SD: ColourmapDefinition = {
  name: 'blue_sd',
  colours: [
    new THREE.Vector3(0.3793034661181724, 0.34040789975053165, 0.7508803052488114),
    new THREE.Vector3(0.3976325589792201, 0.36766544376408467, 0.7680401253342565),
    new THREE.Vector3(0.4156228154081329, 0.3949957866583141, 0.7852253846352517),
    new THREE.Vector3(0.43330864085682713, 0.42242270381844443, 0.8024358462287073),
    new THREE.Vector3(0.4507187072297352, 0.4499637359687955, 0.8196713036101766),
    new THREE.Vector3(0.4678771876257229, 0.47763189848429516, 0.8369315816810977),
    new THREE.Vector3(0.4848046674251862, 0.5054368555932399, 0.8542165322366401),
    new THREE.Vector3(0.5015188348032825, 0.5333857536594, 0.8715260300839889),
    new THREE.Vector3(0.5040038984562412, 0.5375829917347249, 0.8741197228842384),
    new THREE.Vector3(0.5170885938006982, 0.5525019141601941, 0.882853961753396),
    new THREE.Vector3(0.5323785995494978, 0.5701277769082589, 0.893128744041837),
    new THREE.Vector3(0.5475718497453869, 0.5878391347331349, 0.9034063154913805),
    new THREE.Vector3(0.5626776403350616, 0.6056349913772506, 0.9136866720322696),
    new THREE.Vector3(0.5777042443113116, 0.6235143659268424, 0.9239698128904126),
    new THREE.Vector3(0.5926590543077891, 0.6414762936102514, 0.9342557402918829),
    new THREE.Vector3(0.6075487011892828, 0.6595198263679894, 0.9445444591913483),
    new THREE.Vector3(0.6223791533395194, 0.6776440332366411, 0.954835977022395),
    new THREE.Vector3(0.637155800305016, 0.6958480005809768, 0.9651303034679236),
    new THREE.Vector3(0.6518835236673065, 0.7141308322024855, 0.9754274502489376),
    new THREE.Vector3(0.6580562820012059, 0.7218336643657571, 0.979753528291785),
    new THREE.Vector3(0.6643195737343167, 0.726026782802202, 0.9807763715297279),
    new THREE.Vector3(0.6750628468312442, 0.7332708714853562, 0.9825348979380667),
    new THREE.Vector3(0.6857382783269689, 0.7405326716817371, 0.9842869193968725),
    new THREE.Vector3(0.6963495956269842, 0.7478120120517, 0.9860324105350652),
    new THREE.Vector3(0.7069002683726267, 0.7551087252401065, 0.9877713461126013),
    new THREE.Vector3(0.7173935316224211, 0.7624226477436774, 0.9895037010164395),
    new THREE.Vector3(0.7278324064630498, 0.7697536197837083, 0.9912294502565687),
    new THREE.Vector3(0.7350553804708115, 0.7748574819978237, 0.9924246896404804),
    new THREE.Vector3(0.7384989166061473, 0.7776590922595487, 0.9926091935075861),
    new THREE.Vector3(0.7497324708884542, 0.7868564831508208, 0.9932046273064109),
    new THREE.Vector3(0.7608904882492469, 0.7960787031666257, 0.993785870239586),
    new THREE.Vector3(0.7719768150767568, 0.8053254722820444, 0.9943528124573499),
    new THREE.Vector3(0.782995041680365, 0.8145965185193306, 0.9949053436427951),
    new THREE.Vector3(0.7939485245964756, 0.8238915776110411, 0.9954433529918155),
    new THREE.Vector3(0.8048404064940475, 0.8332103926803731, 0.9959667291928435),
    new THREE.Vector3(0.8156736339858098, 0.8425527139376939, 0.9964753604063591),
    new THREE.Vector3(0.8264509736063077, 0.8519182983923415, 0.9969691342441632),
    new THREE.Vector3(0.837175026180444, 0.8613069095788173, 0.9974479377484154),
    new THREE.Vector3(0.8478482397748167, 0.87071831729655, 0.9979116573704113),
    new THREE.Vector3(0.8584729213976448, 0.8801522973624692, 0.9983601789491005),
    new THREE.Vector3(0.8690512475907817, 0.8896086313756596, 0.9987933876893348),
    new THREE.Vector3(0.8795852740383322, 0.8990871064934212, 0.9992111681398302),
    new THREE.Vector3(0.8900769443002639, 0.9085875152181073, 0.9996134041708363),
    new THREE.Vector3(0.9005281293207135, 0.9181095973202068, 0.9999999849571296),
  ],
  underflowColour: new THREE.Vector3(0.0, 0.0, 0.0),
  overflowColour: new THREE.Vector3(1.0, 1.0, 1.0),
};

/** "TR4" via https://sciviscolor.org/ */
export const COLOURMAP_TR4: ColourmapDefinition = {
  name: 'tr4',
  colours: [
    new THREE.Vector3(0.2, 0.168627, 0.160784),
    new THREE.Vector3(0.4, 0.364706, 0.341176),
    new THREE.Vector3(0.65098, 0.623529, 0.584314),
    new THREE.Vector3(0.8, 0.780392, 0.745098),
    new THREE.Vector3(0.901961, 0.886275, 0.854902),
    new THREE.Vector3(1.0, 0.996078, 0.968627),
    new THREE.Vector3(1.0, 0.996078, 0.968627),
    new THREE.Vector3(0.941176, 0.878431, 0.639216),
    new THREE.Vector3(0.878431, 0.733333, 0.47451),
    new THREE.Vector3(0.8, 0.552941, 0.337255),
    new THREE.Vector3(0.721569, 0.376471, 0.231373),
    new THREE.Vector3(0.560784, 0.133333, 0.113725),
  ],
  underflowColour: new THREE.Vector3(0.0, 0.0, 0.0),
  overflowColour: new THREE.Vector3(1.0, 0.0, 1.0),
};

/** "Extended Kindlmann" via www.kennethmoreland.com */
export const COLOURMAP_EXTENDED_KINDLMANN: ColourmapDefinition = {
  name: 'extended_kindlmann',
  colours: [
    new THREE.Vector3(0.0, 0.0, 0.0),
    new THREE.Vector3(0.114621896, 0.012116631, 0.25430111),
    new THREE.Vector3(0.032722432, 0.022251622, 0.466353986),
    new THREE.Vector3(0.016370126, 0.193452028, 0.340869045),
    new THREE.Vector3(0.013577264, 0.281313081, 0.239591784),
    new THREE.Vector3(0.017505711, 0.359608356, 0.103056381),
    new THREE.Vector3(0.088086475, 0.430359056, 0.02076563),
    new THREE.Vector3(0.345901241, 0.475652008, 0.022699138),
    new THREE.Vector3(0.647380038, 0.471950819, 0.031044032),
    new THREE.Vector3(0.963164467, 0.382930018, 0.227508854),
    new THREE.Vector3(0.975618044, 0.489413302, 0.530264615),
    new THREE.Vector3(0.979246791, 0.564484358, 0.884150643),
    new THREE.Vector3(0.882320795, 0.714076956, 0.986232999),
    new THREE.Vector3(0.890857185, 0.820709711, 0.991331242),
    new THREE.Vector3(0.915472162, 0.919226167, 0.995940662),
    new THREE.Vector3(1.0, 1.0, 1.0),
  ],
  underflowColour: new THREE.Vector3(0.0, 0.0, 0.2),
  overflowColour: new THREE.Vector3(1.0, 0.0, 1.0),
};

/**
 * "Plasma" colourmap from matplotlib
 */
export const COLOURMAP_PLASMA: ColourmapDefinition = {
  name: 'plasma',
  colours: [
    new THREE.Vector3(0.0014619955811715805, 0.0004659913919114934, 0.013866005775115809),
    new THREE.Vector3(0.014574183301667168, 0.01165626484047856, 0.07383027500004649),
    new THREE.Vector3(0.04449890545958602, 0.029116239242626116, 0.14520447428014116),
    new THREE.Vector3(0.08562457994077063, 0.04421609649873954, 0.22173241570843438),
    new THREE.Vector3(0.1351485346688386, 0.046903282611846044, 0.2989038128707295),
    new THREE.Vector3(0.1912620160070035, 0.0391958900172573, 0.36223202099711194),
    new THREE.Vector3(0.2473309698973063, 0.03728261717254723, 0.40120279418778),
    new THREE.Vector3(0.30089274611811434, 0.048584344231212905, 0.4214730619540725),
    new THREE.Vector3(0.3528221934608189, 0.06647486565737296, 0.4307728269198571),
    new THREE.Vector3(0.4040952017208467, 0.0856525242127833, 0.43317652187522976),
    new THREE.Vector3(0.45525951586368, 0.10442263501085428, 0.43033023600123377),
    new THREE.Vector3(0.5065140321168304, 0.12263248271162683, 0.42277603858077556),
    new THREE.Vector3(0.5578173383735777, 0.14070006322747505, 0.4105869894838095),
    new THREE.Vector3(0.6089310893911688, 0.15932126640354208, 0.3937379111038313),
    new THREE.Vector3(0.6594450311832232, 0.17937459399911745, 0.37228054249442116),
    new THREE.Vector3(0.7087840459915316, 0.20185769766675088, 0.3464654636019096),
    new THREE.Vector3(0.7562455105458922, 0.22778154674738826, 0.3167472665668382),
    new THREE.Vector3(0.8010224300854427, 0.2580348609536721, 0.28377395969357994),
    new THREE.Vector3(0.8422753716866317, 0.2932294314713286, 0.24827876441296504),
    new THREE.Vector3(0.8792173122104872, 0.33357679192740297, 0.2109315412502884),
    new THREE.Vector3(0.9112366704487734, 0.37883987339379566, 0.17210039136049943),
    new THREE.Vector3(0.9379204275560779, 0.4284761471363702, 0.13175009800625348),
    new THREE.Vector3(0.959041323406337, 0.481797279691003, 0.089671922495064),
    new THREE.Vector3(0.9744679769609869, 0.5381389425344782, 0.04749808541001078),
    new THREE.Vector3(0.9841221863883532, 0.5968995709009532, 0.023699889114392417),
    new THREE.Vector3(0.9878915580475224, 0.6575971476447211, 0.0495984555574076),
    new THREE.Vector3(0.9856470917514636, 0.7197960096749809, 0.11116793361354582),
    new THREE.Vector3(0.9773652656203692, 0.7830035344121836, 0.18691178268761427),
    new THREE.Vector3(0.963797680239549, 0.8463071547230478, 0.2773341527086129),
    new THREE.Vector3(0.9492989663124841, 0.9072802619461583, 0.3885274363634055),
    new THREE.Vector3(0.9512517408257862, 0.9593368448363006, 0.520521918975288),
    new THREE.Vector3(0.9883620799212208, 0.9983616470620554, 0.6449240982803861),
  ],
  underflowColour: new THREE.Vector3(0.0, 0.0, 0.0),
  overflowColour: new THREE.Vector3(1.0, 0.0, 1.0),
};

/**
 * "Fast" by Moreland
 */
export const COLOURMAP_FAST: ColourmapDefinition = {
  name: 'fast',
  colours: [
    new THREE.Vector3(0.05639932216773367, 0.056399092153948, 0.4700000908789252),
    new THREE.Vector3(0.11062118079323027, 0.1380849483224675, 0.5318811940715031),
    new THREE.Vector3(0.15062834225956712, 0.2122970107362595, 0.5947551775157331),
    new THREE.Vector3(0.18364776204016361, 0.28582408330692705, 0.6585723468980588),
    new THREE.Vector3(0.21181580762686186, 0.36018628717252055, 0.723291561166658),
    new THREE.Vector3(0.2360260502066791, 0.4358696051302966, 0.7888773509714441),
    new THREE.Vector3(0.267625116022063, 0.5083081607706341, 0.8350281801403023),
    new THREE.Vector3(0.299465177629453, 0.5797542700808809, 0.8717621559957862),
    new THREE.Vector3(0.32712079411491907, 0.6523755202804778, 0.9084510967262647),
    new THREE.Vector3(0.3512259015236105, 0.7261574853335666, 0.9450952510932998),
    new THREE.Vector3(0.43259949308056317, 0.7774846818972193, 0.9484812495789637),
    new THREE.Vector3(0.5182455112269085, 0.8215939429675145, 0.9401532282112622),
    new THREE.Vector3(0.5934960027213793, 0.8663909235918918, 0.9312813400678497),
    new THREE.Vector3(0.6622681009426095, 0.9118331985033377, 0.9218254834134191),
    new THREE.Vector3(0.7567155629708813, 0.9342020135461144, 0.87542938939741),
    new THREE.Vector3(0.8552551162202264, 0.9411667420787914, 0.8045038976467505),
    new THREE.Vector3(0.9137962604822488, 0.924270570873576, 0.7211958107780932),
    new THREE.Vector3(0.9369921574114255, 0.8836812571866447, 0.6264560093359551),
    new THREE.Vector3(0.9539324119899434, 0.8432846209187139, 0.531408480407559),
    new THREE.Vector3(0.9534516720681238, 0.789549848191143, 0.467841913400552),
    new THREE.Vector3(0.9465758462825115, 0.7316223955774923, 0.4146496642068541),
    new THREE.Vector3(0.9374475033003385, 0.6735004931047588, 0.3619568066943428),
    new THREE.Vector3(0.9245647500491415, 0.6148545383132992, 0.3113167841796553),
    new THREE.Vector3(0.8970517755591756, 0.5546951612479929, 0.2748146276825057),
    new THREE.Vector3(0.8685551960480525, 0.49413078643328173, 0.2389732264611919),
    new THREE.Vector3(0.839074560826857, 0.43274518533936923, 0.20382337636329811),
    new THREE.Vector3(0.8086084101977041, 0.3698342833209696, 0.16939757383932763),
    new THREE.Vector3(0.7676278529327417, 0.315264653443075, 0.15429163686512834),
    new THREE.Vector3(0.72300106561469, 0.26344676525169936, 0.14610655130616268),
    new THREE.Vector3(0.6785270372029161, 0.20934429344113736, 0.13757932265353492),
    new THREE.Vector3(0.6341969500479122, 0.1502395759815047, 0.12870449492087047),
    new THREE.Vector3(0.5900001145322249, 0.07669636770019067, 0.11947505935767005),
  ],
  underflowColour: new THREE.Vector3(0.0, 0.0, 0.2), // Navy blue
  overflowColour: new THREE.Vector3(1.0, 0.0, 1.0), // Magenta
};

/**
 * Elevation terrain colourmap: grey rocky body appearance
 * Used for terrain visualisation mode
 */
export const COLOURMAP_TERRAIN_ELEVATION: ColourmapDefinition = {
  name: 'elevation_terrain',
  colours: [
    new THREE.Vector3(0.2, 0.185098, 0.167255), // Lowest elevation
    new THREE.Vector3(0.258824, 0.243922, 0.238235),
    new THREE.Vector3(0.321569, 0.306667, 0.293137),
    new THREE.Vector3(0.380392, 0.369412, 0.358039),
    new THREE.Vector3(0.439216, 0.426078, 0.412941),
    new THREE.Vector3(0.509804, 0.48451, 0.479608),
    new THREE.Vector3(0.65098, 0.631373, 0.632941),
    new THREE.Vector3(0.74902, 0.737255, 0.72098),
    new THREE.Vector3(0.858824, 0.858824, 0.842549),
    new THREE.Vector3(0.988235, 0.988235, 0.96902), // Highest elevation
  ],
  underflowColour: new THREE.Vector3(0.0, 0.0, 0.2), // Navy blue
  overflowColour: new THREE.Vector3(1.0, 0.0, 1.0), // Magenta
};

/**
 * Water depth colourmap: ocean blues
 * Used for terrain visualisation mode to show water depth
 */
export const COLOURMAP_TERRAIN_WATER: ColourmapDefinition = {
  name: 'water_depth_terrain',
  colours: [
    new THREE.Vector3(0.329412, 0.639216, 0.756863), // Shallowest water, 0m
    new THREE.Vector3(0.120824, 0.35549, 0.46098), // 250m
    new THREE.Vector3(0.020157, 0.169608, 0.271176), // 500m
    new THREE.Vector3(0.019216, 0.152549, 0.262353),
    new THREE.Vector3(0.017615, 0.146676, 0.258724),
    new THREE.Vector3(0.016014, 0.140803, 0.255096),
    new THREE.Vector3(0.014413, 0.13493, 0.251468),
    new THREE.Vector3(0.012812, 0.129057, 0.24784),
    new THREE.Vector3(0.011211, 0.123184, 0.224212),
    new THREE.Vector3(0.00961, 0.117311, 0.210583),
    new THREE.Vector3(0.008009, 0.111438, 0.206955),
    new THREE.Vector3(0.006409, 0.105565, 0.193327),
    new THREE.Vector3(0.004808, 0.099692, 0.189699),
    new THREE.Vector3(0.003207, 0.093819, 0.176071),
    new THREE.Vector3(0.001606, 0.087945, 0.162443),
    new THREE.Vector3(0.000005, 0.082072, 0.158815),
    new THREE.Vector3(0.0, 0.076199, 0.145186),
    new THREE.Vector3(0.0, 0.070326, 0.131558),
    new THREE.Vector3(0.0, 0.064453, 0.10793),
    new THREE.Vector3(0.0, 0.05858, 0.084302),
    new THREE.Vector3(0.0, 0.052549, 0.062353), // Deepest water
  ],
  underflowColour: new THREE.Vector3(0.0, 0.0, 0.2), // Navy blue
  overflowColour: new THREE.Vector3(0.0, 0.052549, 0.062353), // Deepest water colour
};

/**
 * Ice thickness colourmap: ice sheet whites
 * Used for terrain visualisation mode to show ice thickness
 */
export const COLOURMAP_TERRAIN_ICE: ColourmapDefinition = {
  name: 'ice_thickness_terrain',
  colours: [
    new THREE.Vector3(0.8, 0.82, 0.83), // Thinnest ice
    new THREE.Vector3(0.9, 0.91, 0.92),
    new THREE.Vector3(0.9, 0.9, 0.91),
    new THREE.Vector3(0.9, 0.9, 0.9),
    new THREE.Vector3(0.91, 0.91, 0.91),
    new THREE.Vector3(0.91, 0.92, 0.92),
    new THREE.Vector3(0.92, 0.92, 0.92),
    new THREE.Vector3(0.92, 0.93, 0.93),
    new THREE.Vector3(0.93, 0.93, 0.93),
    new THREE.Vector3(0.93, 0.94, 0.94), // Thickest ice
  ],
  underflowColour: new THREE.Vector3(0.0, 0.2, 0.2), // Cyan
  overflowColour: new THREE.Vector3(0.93, 0.94, 0.94), // Thickest ice colour
};
