...
const calculateMixtureTransmission = (
  ...
  for (const { gas, concentration } of gases) {
    ...
    console.log(
      `${gas}: column = ${gasColumnDensity.toExponential(3)} molecules/cm², max k = ${maxK.toExponential(3)}`
    );
  ...
  console.log(
    `Final transmission: min = ${Math.min(...totalTransmission).toExponential(3)}, samples < 0.99: ${finalTransmission.length}`
  );
...
export const HitranLineExperiment: React.FC = () => {
  ...
  const { wavelengths, transmission } = calculateMixtureTransmission(
    spectra,
  ...
  return (
    ...
    >
      ...
      <div style={{ backgroundColor: 'rgba(255 255 255 / 0.1)', padding: '12px' }}>
        ...
        <p>
          Surface pressure: {atmosphere.surfacePressure_Pa.toFixed(0)} Pa (
          {(atmosphere.surfacePressure_Pa / 101325).toFixed(2)} atm)
        </p>
        <p>Surface gravity: {atmosphere.surfaceGravity_m_s2} m/s²</p>
        <p>
          Mean molecular mass:{' '}
          {(atmosphere.meanMolecularMass_kg * PHYSICS_CONSTANTS.AVOGADRO * 1000).toFixed(1)} g/mol
        </p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        {atmosphere.composition.map((g) => {
          ...
          const allKValues = hasData
            ? spectrum.kDistributions.flatMap((kd) => kd.kValues).filter((v) => v > 0)
            : [];
          ...
          return (
            <div
              key={g.gas}
              style={{ backgroundColor: 'rgba(255 255 255 / 0.1)', padding: '12px' }}
            >
              <h3>{g.gas.toUpperCase()}</h3>
              <p>
                Concentration: {(g.concentration * 100).toFixed(6)}% (
                {(g.concentration * 1e6).toFixed(2)} ppm)
              </p>
      ...
      <div style={{ backgroundColor: 'rgba(255 255 255 / 0.1)', padding: '12px' }}>
        ...
        <p>
          ({(blackbodyWeightedTransmission * 100).toFixed(2)}% of surface radiation escapes to
          space)
        </p>
      </div>
      ...
      {spectra.h2o &&
        (() => {
          const h2oKValues = spectra.h2o.kDistributions
            .flatMap((kd) => kd.kValues)
            .filter((v) => v > 0);
          ...
          return (
            <div>
              ...
              <svg width={width} height={400}>
                <Group left={margin.left} top={margin.top}>
                  <GridRows scale={h2oYScale} width={innerWidth} stroke="rgba(255,255,255,0.1)" />
