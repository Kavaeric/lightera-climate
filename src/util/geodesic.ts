 ...
 */
function projectVectorToFace(P: THREE.Vector3, frame: { n: THREE.Vector3; u: THREE.Vector3; v: THREE.Vector3 }): { x: number; y: number } {
  const { n, u, v } = frame
  ...
  if (z > (g + EPS)) {
    console.warn(`Warning - z (${z * RAD} degrees) exceeds g (${g * RAD} degrees) by ${(z - g) * RAD} degrees.`)
  }
  ...
  if (z > (q + EPS)) {
    console.warn(`Warning - z (${z * RAD} degrees) exceeds q (${q * RAD} degrees) by ${(z - q) * RAD} degrees.`)
  }
 ...
 */
function unprojectFromFace({ x, y }: { x: number; y: number }, frame: { n: THREE.Vector3; u: THREE.Vector3; v: THREE.Vector3 }): THREE.Vector3 {
  const { n, u, v } = frame
...
export class Grid {
  ...
  *[Symbol.iterator]() {
  ...
  static makeRefQuad({ up: { tri, frame } }: { up: { tri: THREE.Triangle; frame: { n: THREE.Vector3; u: THREE.Vector3; v: THREE.Vector3 } } }, N: number) {
